...
pub mod runtime {
    ...
    pub use wapo::env::messages::{HttpHead, HttpResponseHead};
    use wapo::local_contract::query_pink;
    pub use wapo::{
    ...
    async fn get_init_script() -> Result<String> {
        type LangError = u8;
        let myid = wapo::ocall::vmid()?;
        let selector = ink_macro::selector_bytes!("wapo_init_script");
    ...
    pub async fn main_loop() {
        ...
        loop {
            tokio::select! {
                ...
                message = wapo::channel::input_messages().next() => {
...
pub mod runtime {
    pub struct HttpRequest;
    pub use wapo::env::messages::{AccountId, HttpHead, HttpResponseHead};
