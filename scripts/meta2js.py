...
class Enum(Type):
    @classmethod
    def load(cls, t: 'TypeInfo'):
        return cls(t.tdef.get('variants', []))
...
class Message:
    ...
    def from_metadata(cls, contract: 'Contract', msg: dict):
        ...
        input_labels = [arg['label'] for arg in msg['args']]
        output = msg['returnType']['type']
        return Message(contract, msg['label'], int(msg['selector'], 16), inputs, input_labels,
                       output)

    def __init__(self, contract, name, selector, inputs, input_labels, output) -> None:
    ...
    def to_json(self, fields=None):
        ...
        if fields is None:
            return json
        return {k: v for k, v in json.items() if k in fields}
...
def to_js(types, contract, messages):
    registry = '\n'.join(types)
    ...
    class {contract_name} {{
        constructor(address) {{
            this.typeRegistryRaw = {registry!r}
    ...
    for m in messages:
        if m.contract != contract:
            continue
        msg_name = camel_case(m.name).replace('::', '$')
        args = ', '.join(m.input_labels)
        io = json.dumps(m.to_json(fields=['inputs', 'output']))
        js += f'''
        {msg_name}({args}) {{
            const io = {io};
            return invokeContract(this.address, {m.selector}, [{args}], io, this.typeRegistry);
        }}\n'''
        # msg_name = camel_case(m.name)
        # print(f'{msg_name}', m.selector, m.inputs, m.output)
    js += '}\n'
    return js
...
def main():
    ...
    if args.json:
        ...
        types, symbols = compat(all_types, strip=args.strip)
        print(to_js(types, contracts[0], messages))
